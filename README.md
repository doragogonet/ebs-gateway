### 概要
本システムは、複数のRFIDリーダーの動作状況を監視し、RFIDタグの読み取りおよび書き込み操作を行うシステムです。本システムはWebSocket/Socket通信を用いてリアルタイムなデータ通信を実現し、1クライアントが複数のRFIDデバイスに対応した柔軟なアーキテクチャを採用しています。

２、システムアーキテクチャ
```
本システムは以下の層構造で設計されています。
①	クライアント層：JSON形式でリクエストおよびレスポンスを行うユーザーインターフェース。
②	Socketサービス層：WebSocket/Socket通信を管理。
③	アプリケーションAPI層：クライアントからのリクエストを処理し、デバイス操作を提供する統一インターフェース。
④	抽象デバイスインターフェース層：RFIDリーダーの標準操作セットを定義。
⑤	ドライバ適用層：各メーカーのドライバ実装（例：RFID_A会社、RFID_B会社）
⑥	物理デバイス通信層：実際のハードウェアと通信するレイヤー。
```
