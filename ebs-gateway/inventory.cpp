...
void handle_start_inventory_thread(ClientData* data, cJSON* request) {
    ...
    // 启动 RFID 盘点线程
    bool success = dev.Start_Inventory_Thread(start_inventory_thread_callbak, timeout_ms->valueint);
    if (!success) {
...
void handle_stop_inventory_thread(ClientData* data) {
    //  Stop_Inventory_Thread();  // 停止 RFID 盘点线程
    //解锁客户端
    // ------------------------------------------------
    destroy_locks();
    // ------------------------------------------------
...
void handle_start_inventory(ClientData* data, cJSON* request) {
    ...
    int result = dev.start_inventory( &tags, &tagCount, timeout_ms->valueint);
