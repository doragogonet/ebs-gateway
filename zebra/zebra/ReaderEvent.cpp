...
extern TRIGGER_INFO triggerInfo;
extern REPORT_TRIGGERS m_ReportTriggers;
extern TAG_EVENT_REPORT_INFO m_TagEventReportInfo;

BOOLEAN g_AccessSequenceRunning = false;
        ...
        switch (dwStatus) {
            ...
            case WAIT_OBJECT_0 + 3: // //INVENTORY_STOP_EVENT                
                ...
                while (RFID_API_SUCCESS == RFID_GetReadTag(readerHandle, pTagData)) {
                    //标签的 opCode 是 ACCESS_OPERATION_NONE，说明它是盘点（Inventory）操作的结果，
                    //opStatus 是 ACCESS_SUCCESS访问操作成功，
...
int StartInventoryPerform(MEMORY_BANK* memoryBank) {
    ...
    RFID_API_SUCCESS != RFID_PurgeTags(readerHandle, NULL);

    if (g_AccessSequenceRunning) {
        printf("StartInventoryPerform running");
        return -1;
    }
    ...
    if (memoryBank) {
        ...
        readAccessParams.memoryBank = *memoryBank;
        ...
        if (RFID_API_SUCCESS == rfidStatus)
        {
            ...
            if (RFID_API_SUCCESS != rfidStatus)
            {
                printf("RFID_PerformAccessSequence API failed");
