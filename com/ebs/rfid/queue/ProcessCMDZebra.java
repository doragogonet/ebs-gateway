...
public class ProcessCMDZebra extends ProcessCMD {
	...
	public ProcessCMDZebra( Map<String,RfidReadZebra> readerMap, Map<String,RFIDReader> driverMap,List<String> workingReaderList,Map<String,String> inventoryIpMap) {
		
		this.driverMap = driverMap;
		this.readerMap = readerMap;
		this.inventoryIpMap = inventoryIpMap;
		this.workingReaderList = workingReaderList;
		this.cacheThreadPool = Executors.newCachedThreadPool();
	}
	
	@Override
	protected void stopInventory(String token, JSONObject json) throws Exception {
		try {
			String tokenStart = json.getString("TOKEN_START");
			String ips = this.inventoryIpMap.get(tokenStart);
		...
		RfidManagerApplication.redis.sendMessage("{\"RSP\":\"" + Constants.RSP02_STOP + "\",\"TOKEN\":\"" + token + "\",\"STATUS\":\"" + this.status + "\"}",token);
	}
	...
	@Override
	protected void tagWrite(JSONObject json, String token) {
		try {
			...
			for (String ipPort : ipArr) {
    			...
    			if (!this.doTagWrite(ip, port, tagId, pwd, memryBank, data, dataOffset ,dataLength, antArr)) {
    				this.status = "500";
    			}
			}
		} catch (Exception ex) {
		...
		RfidManagerApplication.redis.sendMessage("{\"RSP\":\"" + Constants.TAG_WRITE + "\",\"TOKEN\":\"" + token + "\",\"STATUS\":\"" + this.status + "\"}",token);
		
	}

	private boolean doTagWrite(String ip, String port, String tagId,String pwd,String memryBank, String data, String offset, String length, String[] ants) {
	...
	@Override
	protected void tagRead(JSONObject json, String token) {
		...
		try {
			...
			for (String ipPort : ipArr) {
				...
				String ants = ipPortArr[2];
				String[] antArr = ants.split(",");
				tag = this.doTagRead(ip, port, tagId, pwd, antArr);
    			if (tag == null) {
    				this.status = "500";
    			}
			}
		...
		RfidManagerApplication.redis.sendMessage(
				...
				"\",\"GS1_EPC\":\"" + tag.getGs1Epc() + "\"}",token);
	}
	
	private TagInfo doTagRead(String ip, String port, String tagId,String pwd, String[] ants) {
		if (this.workingReaderList.contains(ip)) {
			return null;
		}
		TagInfo ret = null;
	...
	@Override
	protected void tagLock(JSONObject json, String token) {
		try {
			String ipAnts = json.getString("IP_PORT_ANTS");
			String[] ipArr = ipAnts.split(",");
			String tagId = json.getString("TAG_ID");
			...
			for (String ipPort : ipArr) {
				...
				String ants = ipPortArr[2];
				String[] antArr = ants.split(",");
    			if (!this.doTagLock(ip, port, tagId, pwd, lockFieldSrr, lockPrivilege, antArr)) {
    				this.status = "500";
    			}
			}
		} catch (Exception ex) {
		...
		RfidManagerApplication.redis.sendMessage("{\"RSP\":\"" + Constants.TAG_UNLOCK + "\",\"TOKEN\":\"" + token + "\",\"STATUS\":\"" + this.status + "\"}",token);
		
	}
	
	private boolean doTagLock(String ip, String port, String tagId,String pwd,String[] lockFields, String lockPrivilege, String[] ants) {
		if (this.workingReaderList.contains(ip)) {
			return false;
		}
		boolean ret = false;
	...
	@Override
	protected void startInventory(JSONObject json, String token) {
		...
		try {
			...
			for (String ipPort : ipArr) {
				...
				new Thread() {
	    			public void run() {
	    				try {
							if (ProcessCMDZebra.this.inventoryAndRead(ip, port, token, query) != 0) {
								ProcessCMDZebra.this.status = "500";
							}
						} catch (Exception e) {
							// TODO Auto-generated catch block
							e.printStackTrace();
							...
							lock.lock();
							ProcessCMDZebra.this.completedCount--;
							lock.unlock();
	    				}
	    			};
	    		}.start();
		...
		RfidManagerApplication.redis.sendMessage("{\"RSP\":\"" + Constants.RSP02 + "\",\"TOKEN\":\"" + token + "\",\"STATUS\":\"" + this.status + "\"}",token);
	...
	private int inventoryAndRead(String ip,String port,String token, TagQuery query) throws Exception {
		...
		this.workingReaderList.add(ip);
		String ips = "";
		if (this.inventoryIpMap.containsKey(token)) {
			ips = this.inventoryIpMap.get(token);
			ips += "," + ip;
		} else {
			ips = ip;
		}
		this.inventoryIpMap.put(token, ips);
	...
	@Override
	protected void getRfidDatas(JSONObject json, String token) {
		String datasJson = null;
			try {
    			...
    			datasJson = this.doGetRfidDatas(token, list);
			} catch (Exception ex) {
				ex.printStackTrace();
			}
			if (datasJson != null) {
				RfidManagerApplication.redis.sendMessage(datasJson, token);
			} else {
				RfidManagerApplication.redis.sendMessage("{\"RSP\":\"" + Constants.GET_RFIDDATA + "\",\"TOKEN\":\"" + token + "\",\"RFID\":[],\"STATUS\":\"200\"}",token);
			}
	}
	
	private String doGetRfidDatas(String token, List<RfidData> list) {
		if (list == null) {
			return null;
		}
	...
	@Override
	protected void encodeEpc(JSONObject json, String token) {
		String encode = "";
			try {
			...
			RfidManagerApplication.redis.sendMessage("{\"RSP\":\"" + Constants.EPC_ENCODE + "\",\"TOKEN\":\"" + token + "\",\"ENCODE\":\"" + encode + "\",\"STATUS\":\"" + this.status + "\"}",token);
			
	}
	
	@Override
	protected void decodeEpc(JSONObject json, String token) {
		String decode = "";
			try {
			...
			RfidManagerApplication.redis.sendMessage("{\"RSP\":\"" + Constants.EPC_DECODE + "\",\"TOKEN\":\"" + token + "\",\"DECODE\":\"" + decode + "\",\"STATUS\":\"" + this.status + "\"}",token);
