...
public class ProcessCMDThread extends Thread {
	...
	private Logger logger = Logger.getLogger(ProcessCMDThread.class);
		 ...
		 try {
			 switch (cmd) {
	 			...
	 			case Constants.TAG_LOCK :
		 			...
		 			RfidManagerApplication.redis.sendMessage("{\"RSP\":\"" + Constants.TAG_LOCK + "\",\"TOKEN\":\"" + token + "\",\"STATUS\":\"" + this.status + "\"}",token);
	 ...
	 private void getToken() {
		...
		RfidManagerApplication.redis.sendMessage("{\"RSP\":\"" + Constants.RSP01 + "\",\"TOKEN\":\"" + token + "\",\"STATUS\":\"200\"}",token);
	 }
